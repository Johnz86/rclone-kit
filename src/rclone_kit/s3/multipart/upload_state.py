...
class UploadState:
    ...
    def update_source_file(self, src_file: Path, known_file_size: int | None) -> None:
        new_file_size = (
            known_file_size if known_file_size is not None else os.path.getsize(src_file)
        )
    ...
    def remaining(self) -> int:
        ...
        assert count <= num_chunks, f"Count {count} is greater than num_chunks {num_chunks}"
    ...
    def from_json(s3_client: BaseClient, json_file: Path) -> "UploadState":
        ...
        return UploadState(peristant=json_file, upload_info=upload_info, parts=finished_parts)
