...
`pathlib`-like remote filesystem interface (`filesystem`/`RemoteFS`).
"""
...
def test_serve_http_download_multi_threaded_reassembles_the_file(
    live_rclone: Rclone, live_test_prefix: str, tmp_path: Path
) -> None:
    payload = bytes(range(256)) * 100
    live_rclone.write_bytes(payload, f"{live_test_prefix}/large.bin")
    outfile = tmp_path / "large.bin"
